...
/*
	자바빈(JavaBean);자바 클래스 >> 스프링에서 빈의 의미
	자바빈 규약에 따라 작성된 클래스이다.
	예를 들어 회원정보, 게시판 글 등의 정보를 출력할 때 정보를 저장하고 있는 자바빈 객체를 사용하게 된다.
	
	자바빈의 규약
	1. 필드마다 별도의 get/set 메서드가 존재해야 한다.
	2. get 메서드는 파라미터가 존재하지 않아야한다.
	3. set 메서드는 반드시 하나 이상의 파라미터가 존재해야 한다.
	4. 빈즈 컴포넌트의 속성은 반드시 읽기 또는 쓰기가 가능해야 한다.
		단, 읽기 전용인 경우 get 또는 is 메서드만 정의할 수 있다.
	5. 생성자는 파라미터가 없는 기본생성자를 반드시 작성해 주어야 한다.
	6. 필드의 접근 제어자는 private이고 각 set/get 메서드의 접근 제어자는 public으로
		정의되어야 하며 클래스의 접근 제어자는 public으로 정의한다.
	
	*자바빈 클래스는 데이터를 저장하는 필드, 데이터를 읽어올 때 사용되는
	 메서드(getter메서드), 값을 저장할 때 사용되는 메서드(setter메서드)로 구성된다.
	
	*메서드 이름을 사용해서 프로퍼티(property)의 이름을 결정하게 된다.
	 예를 들어 프로퍼티의 이름이 name이고 값의 타입이 int인 경우 프로퍼티와 관련된 메서드의 이름은 다음과 같이 결정됨
	 public void setName(int value){}'
	 public int getName(){};
	 
	 setName()과 getName()을 통해 관리되는 데이터를 프로퍼티(property)라고 부른다.
	 프로퍼티의 타입이 boolean인 경우 get대신 is를 붙일 수 있다.
